...
pub fn main(args: Args, paths: &crate::Paths) -> anyhow::Result<()> {
    let dir = paths.run.clone();
    let out = match args.out {
        Some(path) => path,
        None => paths.analysis.join(FILE),
    };

    let ran = Ledger::load(&dir)?;
    ledger::warn(ran.failed(), "command(s)");

    // the rung sizes are what the build counted, read back off the set rather
    // than measured again
    let set = util::set::Set::load_as(&paths.set, crate::SHAPE)?;
    let sizes: BTreeMap<usize, u64> = set
        .units()
...
fn points(ran: &Ledger, sizes: &BTreeMap<usize, u64>) -> anyhow::Result<Vec<Point>> {
    ...
    for row in ran.rows() {
        ...
        let t_res = *sizes
            .get(&t_rung)
            .with_context(|| format!("the set names no target rung {t_rung}"))?
            as f64;
