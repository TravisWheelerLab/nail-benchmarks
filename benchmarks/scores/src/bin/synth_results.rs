...
fn main() -> anyhow::Result<()> {
    ...
    set(&args.out, &shards, args.pairs)?;
...
/// The manifest a real build would have written, since nothing here writes the
/// sequences themselves. `parse` reads the shard sizes off it, so the numbers
/// have to be there even though the files behind them are not.
fn set(dir: &Path, shards: &[String], pairs: usize) -> anyhow::Result<()> {
    let count = pairs / 10;

    let rows: Vec<util::set::Row> = shards
        .iter()
        .map(|shard| {
            util::set::Row::new(shard, format!("targets/{shard}.fa"))
                .query_hmm("query.hmm")
                .attr("shard", shard)
                .attr("seqs", count)
                .attr("residues", count as u64 * 320)
                .attr("bytes", count as u64 * 340)
        })
        .collect();

    util::set::Set::new(dir, rows)
        .says("shape", util::set::shape::FIXED.name)
        .says("recipe", "synthetic")
        .save()
}
