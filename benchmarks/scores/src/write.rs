...
mod tests {
    ...
    fn pipeline(dir: &Path) {
        ...
        // the manifest a build would have left: parse reads the shard sizes
        // off it rather than counting the files again
        util::set::Set::new(
            dir,
            [("1", 3usize, 12u64, 78u64), ("2", 2, 8, 46)]
                .into_iter()
                .map(|(shard, seqs, residues, bytes)| {
                    util::set::Row::new(shard, format!("targets/{shard}.fa"))
                        .query_hmm("query.hmm")
                        .query_sto("query.sto")
                        .query_db("queryDB/queryDB")
                        .attr("shard", shard)
                        .attr("seqs", seqs)
                        .attr("residues", residues)
                        .attr("bytes", bytes)
                })
                .collect(),
        )
    ...
    fn collected(dir: &Path, out: &Path, threads: usize) -> String {
        let set = util::set::Set::load_as(dir, &util::set::shape::FIXED).unwrap();
