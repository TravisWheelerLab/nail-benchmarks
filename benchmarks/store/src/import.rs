...
#[derive(Parser, Debug)]
pub struct Args {
    /// The run directory the tables came back into
    #[arg(long, value_name = "dir")]
    run: PathBuf,

    /// The set these results were searched against
    #[arg(long, value_name = "dir")]
    set: PathBuf,
...
impl Run {
    ...
    fn check_targets(&self, set: &Set, named: &str) -> anyhow::Result<()> {
        let units: Vec<&str> = set.units().map(|u| u.name()).collect();

        let missing: Vec<&str> = self
            .shards
            .iter()
            .map(|shard| shard.name.as_str())
            .filter(|shard| !units.contains(shard))
            .collect();

        ensure!(
            missing.is_empty(),
            "run {:?} covers {} shard(s) the set {:?} does not name: {}\n\n\
             the build has to have made the same shards these were searched against",
            self.name,
            missing.len(),
            named,
            missing.join(", ")
