#! /usr/bin/python3

from common import HmmIndex

import sys
import os


def write(hmm_path, out_dir, index):
    os.makedirs(out_dir, exist_ok=True)
    _, ext = os.path.splitext(hmm_path)

    hmm_lines = [line for line in open(hmm_path)]

    for (start, end, _, name) in index.ranges:
        with open(f"{out_dir}/{name}{ext}", "w") as f:
            for line in hmm_lines[start:end + 1]:
                f.write(line)


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("usage: hmmsplit <file> [out_dir]")
        sys.exit(1)

    hmm_path = sys.argv[1]
    index = HmmIndex(hmm_path)

    if len(sys.argv) > 2:
        out_dir = sys.argv[2]
    else:
        name, _ = os.path.splitext(hmm_path)
        out_dir = f"{name}-splits"

    write(hmm_path, out_dir, index)
